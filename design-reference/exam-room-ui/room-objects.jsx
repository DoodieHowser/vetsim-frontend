// room-objects.jsx — clickable furniture artwork (painterly div+gradient set
// pieces), tuned to read like real GP-clinic equipment. Each fills its Hotspot
// box (inset:0). No interaction logic here — the scene wraps them in <Hotspot>.

// ─────────── Microscope (on counter) → Diagnostics ───────────
function Microscope() {
  return (
...
// ─────────── Pharmacy shelf (wall) → Oral medication ───────────
// Open white melamine shelving, columns of varied bottles & boxes.
function PharmacyCabinet() {
  const caps = ["#C4623E", "#5B8C7B", "#7C9CC4", "#D9A24C", "#B58BB0", "#6FA77C", "#D98C5F", "#8AA9C4"];
  // each row: list of [bodyTint, capColor, heightPct]
  const rows = [
...
// ─────────── Mini fridge (on counter / on top of cabinet) → Injectables ───────────
// Black benchtop vaccine fridge.
function MiniFridge() {
  return (
...
// ─────────── Computer (on counter) → Patient Chart ───────────
// Realistic flat monitor with a blue desktop + a keyboard on the counter.
function ComputerScreen({ count = 0 }) {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      {/* keyboard on counter */}
        ...
        <div style={{ position: "absolute", inset: 5, borderRadius: 3, overflow: "hidden", background: "linear-gradient(160deg,#2b6fb0,#15467e 70%,#0e3460)" }}>
          ...
          <div style={{ position: "absolute", left: "10%", top: "12%", right: "16%", bottom: "26%", borderRadius: 3, background: "rgba(252,250,245,0.96)", boxShadow: "0 3px 8px rgba(0,0,0,0.3)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "4px 6px", display: "flex", flexDirection: "column", gap: 3 }}>
              {/* patient record lines (signalment + notes) */}
...
// ─────────── Clinic door (back wall) → Disposition ───────────
function ClinicDoor() {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
...
// ─────────── Wall clipboard → Differentials ───────────
function WallClipboard({ count = 0 }) {
  return (
    <div style={{ position: "absolute", inset: 0 }}>
...
Object.assign(window, { Microscope, PharmacyCabinet, MiniFridge, ComputerScreen, ClinicDoor, WallClipboard, Otoscope, Ophthalmoscope });

// ─────────── Wall charger station (scenery: plate + cradles + coiled cords) ───────────
function InstrumentCharger() {
  const coil = (cx, loops) => {
    const out = [];
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      <svg width="100%" height="100%" viewBox="0 0 132 168" preserveAspectRatio="xMidYMin meet" style={{ display: "block", overflow: "visible" }}>
        {/* coiled cords hanging from each cradle */}
        {coil(44, 17)}
        {coil(88, 17)}
        {/* white wall plate */}
...
// ─────────── Ophthalmoscope (clickable) → eyes exam ───────────
function Ophthalmoscope() {
  return (
    <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "start center" }}>
      <svg width="100%" height="100%" viewBox="0 0 48 112" preserveAspectRatio="xMidYMin meet" style={{ display: "block", overflow: "visible" }}>
        <defs>
...
window.InstrumentCharger = InstrumentCharger;
window.Otoscope = Otoscope;
window.Ophthalmoscope = Ophthalmoscope;
